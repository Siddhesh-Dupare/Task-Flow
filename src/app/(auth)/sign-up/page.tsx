...
import { User, Mail, Lock, Eye, EyeOff, CheckCircle, CircleDot } from 'lucide-react';
...
const formSchema = z.object({
  name: z.string().trim().refine(name => {
  ...
  password: z.string()
    ...
    .refine(password => /[a-z]/.test(password) && /[A-Z]/.test(password), "Must contain both lowercase and uppercase letters.")
    .refine(password => /[0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+/.test(password), "Must contain a number or a symbol."),
  ...
  return (
    <div className="w-full max-w-md mx-auto p-8 bg-white rounded-lg shadow-xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500">Secure Your Communications with Task Flow</p>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormItem>
                <FormControl>
                  <div className="relative flex items-center">
                    ...
                    <Input placeholder="First Name & Last Name" {...field} className="pl-10 pr-10 py-3" />
              ...
              <FormItem>
                <FormControl>
                  <div className="relative flex items-center">
                    ...
                    <Input type="email" placeholder="Email address" {...field} className="pl-10 pr-10 py-3" />
                    {fieldState.isDirty && fieldState.error === undefined && (
                      <CheckCircle className="absolute right-3 h-5 w-5 text-green-500" />
                    )}
                  </div>
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <div className="mt-2 ml-2">
                  <PasswordValidationItem isValid={passwordValue.length >= 8} text="Least 8 characters" />
                  <PasswordValidationItem isValid={/[0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+/.test(passwordValue)} text="Least one number or symbol" />
                  <PasswordValidationItem isValid={/[a-z]/.test(passwordValue) && /[A-Z]/.test(passwordValue)} text="Lowercase & uppercase letters" />
                </div>
                <FormMessage />
              </FormItem>
            )}
            ...
            render={({ field, fieldState }) => (
              <FormItem>
                <FormControl>
                  <div className="relative flex items-center">
                    <Lock className="absolute left-3 h-5 w-5 text-gray-400" />
                    <Input type="password" placeholder="Re-Type Password" {...field} className="pl-10 pr-10 py-3" />
          ...
          <Button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-md flex items-center justify-center space-x-2 text-lg">
            {isLoading && (
